...
export class UnoGame {
  ...
  public removePlayer(username: string): void {
    const index = this.players.findIndex(p => p.username.toLowerCase() === username.toLowerCase());
    if (index !== -1) {
      ...
      if (this.status === 'playing') {
        if (this.players.length < 2) {
          this.endGame(this.players[0]?.username || null, 'Not enough players remaining.');
        } else {
  ...
  public setReady(username: string, ready: boolean): void {
    ...
    if (player && this.status === 'lobby') {
      ...
      this.addLog(`${username} is ${ready ? 'ready' : 'not ready'}.`);
    }
  }
  ...
  public startGame(): boolean {
    ...
    while (startingCard && (startingCard.color === 'Wild' || startingCard.value === 'Wild' || startingCard.value === 'Wild4')) {
    ...
    if (startingCard) {
      ...
      this.addLog(`Game started! First card is ${startingCard.color} ${startingCard.value}.`);
  ...
  private applyStartingCardEffect(card: Card) {
    ...
    if (card.value === 'Skip') {
      this.addLog(`${activePlayer.username}'s turn was skipped by the starting Skip card!`);
      this.advanceTurn();
    } else if (card.value === 'Reverse') {
      this.playDirection = -1;
      this.turnIndex = this.players.length - 1; // Reverse direction means player 0 is skipped or we start from the last player
      this.addLog(`Play direction reversed by starting Reverse card!`);
    } else if (card.value === 'Draw2') {
      this.addLog(`${activePlayer.username} must draw 2 cards and skip their turn!`);
  ...
  private initializeDeck(): void {
    ...
    for (const color of colors) {
      ...
      for (let i = 1; i <= 9; i++) {
        this.deck.push(createCard(color, i.toString() as CardValue));
        this.deck.push(createCard(color, i.toString() as CardValue));
      }

      // Two of each action card
      for (const action of ['Skip', 'Reverse', 'Draw2'] as CardValue[]) {
        this.deck.push(createCard(color, action));
        this.deck.push(createCard(color, action));
      }
    }
    ...
    for (let i = 0; i < 4; i++) {
      this.deck.push(createCard('Wild', 'Wild'));
      this.deck.push(createCard('Wild', 'Wild4'));
    }
  }
  ...
  public isPlayerTurn(username: string): boolean {
    ...
    if (this.wildPendingColorSelectionBy) {
      return this.wildPendingColorSelectionBy.toLowerCase() === username.toLowerCase();
    }
    return this.getActivePlayer().username.toLowerCase() === username.toLowerCase();
  }

  public canPlayCard(username: string, cardId: string): boolean {
  ...
  public playCard(username: string, cardId: string): boolean {
    ...
    this.addLog(`${username} played ${card.color === 'Wild' ? '' : card.color + ' '}${card.value}.`);
  ...
  public selectWildColor(username: string, color: CardColor): boolean {
    if (this.status !== 'playing') return false;
    if (this.wildPendingColorSelectionBy !== username) return false;
    ...
    if (topCard && topCard.value === 'Wild4') {
      ...
      this.addLog(`${nextPlayer.username} must draw 4 cards and skip their turn!`);
  ...
  private resolveCardAction(card: Card) {
    if (this.checkWinCondition(this.getActivePlayer().username)) return;

    if (card.value === 'Skip') {
      const nextPlayer = this.players[this.getNextPlayerIndex()];
      this.addLog(`${nextPlayer.username}'s turn was skipped!`);
      ...
      if (this.players.length === 2) {
        ...
        this.playDirection = (this.playDirection * -1) as 1 | -1;
        this.addLog(`Play direction is now ${this.playDirection === 1 ? 'clockwise' : 'counter-clockwise'}.`);
        this.advanceTurn();
      }
    } else if (card.value === 'Draw2') {
      const nextPlayerIndex = this.getNextPlayerIndex();
      const nextPlayer = this.players[nextPlayerIndex];
      this.addLog(`${nextPlayer.username} must draw 2 cards and skip their turn!`);
  ...
  public catchUnoFailure(catcherUsername: string, targetUsername: string): boolean {
    if (this.status !== 'playing') return false;
    ...
    if (target.hand.length === 1 && !this.unoShouted[target.username]) {
      this.addLog(`${catcherUsername} caught ${targetUsername} neglecting to shout UNO!`);
      this.addLog(`${targetUsername} draws 2 cards as a penalty.`);
  ...
  private playerDrawCards(username: string, count: number, logIt: boolean = true) {
    const player = this.getPlayer(username);
    if (!player) return;
  ...
  private endGame(winnerUsername: string | null, reason?: string) {
    this.status = 'ended';
    this.winner = winnerUsername;
    if (winnerUsername) {
      this.addLog(`Game ended! Winner is ${winnerUsername}!`);
      // Update database statistics
      for (const player of this.players) {
        db.incrementUserStats(player.username, player.username === winnerUsername);
      }
    } else {
  ...
  public getGameState(playerUsername?: string): GameState {
    return {
      ...
      players: this.players.map(p => {
        const isCurrentRequester = playerUsername && p.username.toLowerCase() === playerUsername.toLowerCase();
        return {
          username: p.username,
          isReady: p.isReady,
          ...
          hand: this.status === 'playing' && !isCurrentRequester 
            ? Array(p.hand.length).fill({ id: '', color: 'Wild', value: 'Wild' }) // Placeholder cards
            : p.hand
        };
      }),
      ...
      wildPendingColorSelectionBy: this.wildPendingColorSelectionBy,
