...
import { db } from './db';
import { generateToken, authenticateJWT, authenticateSocket, AuthenticatedRequest } from './auth';
      ...
      if (socket) {
        ...
        if (session) {
          if (session.deviceType === 'mobile') {
            ...
            socket.emit('game-state', game.getGameState(session.username));
          } else {
            // PC client gets state with all hands obscured
            socket.emit('game-state', game.getGameState(undefined));
          }
        }
      }
    }
...
// Lobby: Create game
app.post('/api/lobby/create', authenticateJWT, (req: AuthenticatedRequest, res) => {
  if (!req.user) return res.status(401).json({ error: 'Unauthorized' });
...
app.get('/api/lobby/list', authenticateJWT, (req, res) => {
...
app.get('/api/lobby/check/:gameId', authenticateJWT, (req: AuthenticatedRequest, res) => {
  ...
  if (game.players.length >= 4 && !alreadyIn) {
    return res.status(400).json({ error: 'Game lobby is full (max 4 players)' });
  }

  res.json({
    gameId: game.gameId,
    status: game.status,
    players: game.players.map(p => ({ username: p.username, isReady: p.isReady })),
  });
});
...
io.on('connection', (socket: any) => {
  ...
  socket.on('join-game', ({ gameId, deviceType }: { gameId: string; deviceType: 'pc' | 'mobile' }) => {
    const game = lobbyManager.getGame(gameId);
    if (!game) {
    ...
    if (!success) {
      socket.emit('error-msg', 'Cannot join game. Lobby full or already playing.');
      return;
    }

    // Register connection in Lobby Manager
    const registered = lobbyManager.registerSocket(socket.id, username, gameId, deviceType);
    if (!registered) {
      socket.emit('error-msg', 'Failed to register device session.');
      return;
    }
    ...
    console.log(`[Socket] User ${username} connected via ${deviceType} to game ${game.gameId}`);
  ...
  socket.on('start-game', () => {
    ...
    if (started) {
      ...
      socket.emit('error-msg', 'Cannot start game. Make sure all players are ready (min 2, max 4 players).');
    }
  });
  ...
  socket.on('play-card', ({ cardId }: { cardId: string }) => {
    ...
    if (success) {
      ...
      socket.emit('error-msg', 'Invalid play. Not your turn or card cannot be played.');
    }
  });
  ...
  socket.on('select-color', ({ color }: { color: any }) => {
    ...
    if (success) {
      ...
      socket.emit('error-msg', 'Failed to choose color. Are you the active player?');
    }
  });
  ...
  socket.on('draw-card', () => {
    ...
    if (drawnCard) {
      ...
      socket.emit('error-msg', 'Cannot draw card. Not your turn or you already drew a card.');
    }
  });
  ...
  socket.on('pass-turn', () => {
    ...
    if (success) {
      ...
      socket.emit('error-msg', 'Cannot pass turn. Did you draw a card first?');
    }
  });
  ...
  socket.on('shout-uno', () => {
    ...
    if (success) {
      ...
      socket.emit('error-msg', 'Cannot shout UNO unless you have exactly 1 card.');
    }
  });

  socket.on('catch-uno', ({ targetUsername }: { targetUsername: string }) => {
    const session = lobbyManager.getSession(socket.id);
    if (!session) return;
    ...
    const success = game.catchUnoFailure(username, targetUsername);
    if (success) {
      broadcastGameState(game.gameId);
    } else {
      socket.emit('error-msg', 'Cannot catch player. Either they have shouted UNO already, or they have more than 1 card.');
    }
  });
  ...
  socket.on('disconnect', () => {
    ...
    if (session) {
      console.log(`[Socket] User ${username} disconnected their ${session.deviceType} from game ${session.gameId}`);
      ...
      if (game) {
        ...
        const allDisconnected = game.players.every(p => !p.isConnectedPC && !p.isConnectedMobile);
        if (allDisconnected) {
          console.log(`[Socket] All players disconnected from game ${game.gameId}. Cleaning up game.`);
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`LAN network: http://${getLocalIpAddress()}:${PORT}`);
  console.log(`=========================================`);
});
